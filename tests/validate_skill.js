...
assert.ok(/authorization link/i.test(skill), 'SKILL.md must explain automatic browser authorization')
...
assert.ok(/auth_sessions_url/.test(script), 'script must support creating authorization sessions')
assert.ok(/webbrowser\.open/.test(script), 'script must open the authorization link when no token is configured')
...
assert.ok(/auth\/sessions/.test(apiReference), 'API reference must document browser authorization sessions')
